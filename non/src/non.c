...
static int non_evo_exc(
    struct pcut *rec, struct pcut *fun, struct vec *xk, struct vec *fk, struct imtx *jk, struct non_new_ops *ops)
{
    if (fun->len < xk->n) {
        errno = EINVAL;
        return -1;
...
int non_new_slv(struct pcut *fun, struct vec *x, struct non_new_ops ops)
{
    ...
    if (ops.cbk)
        ops.cbk->run(ops.cbk->ctx, 1, ops.itr);
