    ...
    for (int i = 0; i < ns; ++i) {
        ...
        for (int i = i0; i <= i1; i += st) {
            ...
            switch (ctx->cnd.mod) {
                case CND_DIR:
                    m->ad[i][0] = 1;
                    f->dat[i] = ctx->cnd.pps.dir.tmp(0, &v);
                ...
                case CND_NEU:
                    ...
                    f->dat[i] += ctx->cnd.pps.neu.tta(0, &v);
                ...
                case CND_ROB:
                    ...
                    double bet = ctx->cnd.pps.rob.bet(0, &v);

                    f->dat[i] += bet * ctx->cnd.pps.rob.tmp(0, &v);
    ...
    for (int i = 0; i < nq; ++i) {
        ...
        for (int i = y0, n = a; i <= y1; ++i, n += dx) {
            ...
            for (int j = x0; j <= x1; ++j, ++n) {
                ...
                double lam = ctx->lam(0, &v);
                double gam = ctx->gam(0, &v);
                double ext = ctx->ext(0, &v);
                ...
                m->ad[n][0] = 2 * lam * (1 / (hl * hr) + 1 / (hd * hu)) + gam;
                m->ad[n][1] = -2 * lam / (hr * (hr + hl));
                m->ad[n][2] = -2 * lam / (hu * (hu + hd));
                m->ad[n][3] = -2 * lam / (hl * (hr + hl));
                m->ad[n][4] = -2 * lam / (hd * (hu + hd));
