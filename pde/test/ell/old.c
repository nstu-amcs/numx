...
int main()
{
    ...
    for (int i = 0; usr[i] != NULL; ++i) {
        ...
        sim_imp_elm(&sim, "pde/test/ell/case0/elm/case.sif");
        ...
        sprintf(fname, "%s/%s.slv", sim.ops.exp.dir, usr[i]);
        ctx.fs = fopen(fname, "w+");
...
static void on_slv(void *c, struct sim *sim)
{
    ...
    fprintf(ctx->fs, "%d & %.7e & %d & %.7e\n", sim->slv->run.ti, err / num, ctx->non.itr, ctx->non.err);
}
